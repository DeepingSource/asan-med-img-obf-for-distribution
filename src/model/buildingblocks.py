""" Credit: https://github.com/wolny/pytorch-3dunet """
...
class Encoder(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, conv_kernel_size=3, apply_pooling=True,
                 ...
                 num_groups=8, padding=1, drop_out=False):
...
class Decoder(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, conv_kernel_size=3, scale_factor=(2, 2, 2), basic_module=DoubleConv,
        ...
        if upsample:
            if basic_module == DoubleConv:
                ...
                if upsample_type == 'trans_conv':
                    self.upsampling = TransposeConvUpsampling(in_channels=in_channels, out_channels=out_channels,
                                                              kernel_size=conv_kernel_size, scale_factor=scale_factor)
                elif upsample_type == 'interpolation':
                    self.conv = nn.Conv3d(in_channels, out_channels, kernel_size=conv_kernel_size, padding=1)
                    self.upsampling = InterpolateUpsampling(mode='trilinear')
                else:
                    raise ValueError
                # sum joining
    ...
    def forward(self, encoder_features, x):
        if self.upsample and self.basic_module != DoubleConv and self.upsample_type == 'interpolation':
            x = self.conv(x)
...
def create_encoders(in_channels, f_maps, basic_module, conv_kernel_size, conv_padding, layer_order, num_groups,
                    pool_kernel_size, drop_out, pool_type):
